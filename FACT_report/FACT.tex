...
Therefore, in the process of unfolding, data of real observations can be corrected using calculations presented in the following chapter. 
    ...
    %
    \section{Theory}
    The energy spectrum is given by the distribution f(E) which represents the distribution of energy of individual particles originating from a given source.
    For experimental data, this distribution can not be measured diectly but represented by a histogram containing individual measurements in the form of an n-dimensional vector X.
    In real measurements the results diverge from the true values of X and are therefore the different vector Y of dimension m.
    Connecting these two we find a nxm matix A representing the probabilities of one count to migrate from one energy bin to another, in other words the process of measuring.
%
% -- Aufbau -- %
%
